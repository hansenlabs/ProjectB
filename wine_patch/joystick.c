...
MMRESULT WINAPI DECLSPEC_HOTPATCH joyGetDevCapsW( UINT_PTR id, JOYCAPSW *caps, UINT size )
{
    ...
    if (!(device = joysticks[id].device) && (ever_checked == 0) && (ticks - last_check) >= 2000)
    {
        last_check = ticks;
        find_joysticks();
        ever_checked=1;
    ...
    else
    {
        ...
        if (FAILED(hr)) 
        {
            wcscpy( caps->szPname, L"Name Lookup failed" );
        }
        else wcscpy( caps->szPname, prop_string.wsz );
        
        if (wcslen(caps->szPname)>=20)caps->szPname[20]=0; //truncate
