...
#define GTEST(cond, flag)                                           \
  do {                                                              \
    if (cond) {                                                     \
      if (!flag) clock_gettime(CLOCK_MONOTONIC, &d->gesture_start); \
      flag = 1;                                                     \
    } else                                                          \
      flag = 0;                                                     \
  } while (0)
...
void draw_gamepad(int y, int x,        // Position oben links
...
static void draw_ui() {
  ...
  mvprintw(0, 1,
           "Connect Keyboards,Gamepads, check layout, hold Up+B1 to start! (F1 "
           "toggle Help)");
  ...
  if (toggle_help) {
    ...
    mvprintw(
        y++, 2,
        "Change Key2Joy Layout: type: LCTRL,k,up,down,left,right,B1,B2,LCTRL");
    mvprintw(y++, 2,
             "Default Layout: Arrow Keys, Number 1, Number 2 (left ones, not "
             "numpad)");
    mvprintw(y++, 2,
             "Toggle Key2Joy mute / unmute LCTRL (prevent double events)");
    mvprintw(y++, 2,
             "Gestures Up+B1 - Atomic Bomberman; Down+B2 Safe Shutdown; "
             "Right+B2 Terminal");
    mvprintw(y++, 2,
             "Type=Hit/release 1 key at time; Gestures: keep 2 pressed.");
  ...
  for (int i = 0; i < dev_count; ++i) {
    ...
    double dt = (now.tv_sec - d->gesture_start.tv_sec) +
                (now.tv_nsec - d->gesture_start.tv_nsec) / 1e9;
    if (42) {
      if (d->gesture && (dt > 0.5)) {
        char temp[64];
        ...
        draw_gamepad(y, x, d->name, d->axis[0], d->axis[1], d->btn[0],
                     d->btn[1]);
      }
      ...
      // arrows
      attron(COLOR_PAIR(up ? CP_ACTIVE : CP_DIM));
      mvprintw(y, 25, "%s", SYM_UP);
      attroff(COLOR_PAIR(up ? CP_ACTIVE : CP_DIM));
      attron(COLOR_PAIR(down ? CP_ACTIVE : CP_DIM));
      mvprintw(y, 27, "%s", SYM_DOWN);
      attroff(COLOR_PAIR(down ? CP_ACTIVE : CP_DIM));
      attron(COLOR_PAIR(left ? CP_ACTIVE : CP_DIM));
      mvprintw(y, 29, "%s", SYM_LEFT);
      attroff(COLOR_PAIR(left ? CP_ACTIVE : CP_DIM));
      attron(COLOR_PAIR(right ? CP_ACTIVE : CP_DIM));
      mvprintw(y, 31, "%s", SYM_RIGHT);
      attroff(COLOR_PAIR(right ? CP_ACTIVE : CP_DIM));

      // buttons
      attron(COLOR_PAIR(d->btn[0] ? CP_ACTIVE : CP_DIM));
      mvprintw(y, 35, "[B1]");
      attroff(COLOR_PAIR(d->btn[0] ? CP_ACTIVE : CP_DIM));
      attron(COLOR_PAIR(d->btn[1] ? CP_ACTIVE : CP_DIM));
      mvprintw(y, 39, "[B2]");
      attroff(COLOR_PAIR(d->btn[1] ? CP_ACTIVE : CP_DIM));
...
int main(void) {
  ...
  int wd = inotify_add_watch(
      inotify_fd, DEV_DIR,
      IN_CREATE | IN_MOVED_TO | IN_ATTRIB | IN_DELETE | IN_MOVED_FROM);
  if (wd < 0) {
    endwin();
    ...
    if (sel > 0 && FD_ISSET(inotify_fd, &readfds)) {
      ...
      if (len > 0) {
        ...
        while (i < len) {
          ...
          if (ev->len > 0 &&
              strncmp(ev->name, JS_PREFIX, strlen(JS_PREFIX)) == 0) {
            char full[512];
