...
Keep the terminal alive. Key2joy supports hotplugging, but Atomic Bomberman does not.
...
-Do not assign keys that will exit the game (Key Z, F-Keys)
```
...
### Switch back to X11 (and probably encounter the next bug)
...
Solution 2 Install / Use a different Window Manager (e.g. Openbox)
...
This software is a modified version of Wine, where all the Wine components are compiled for ARM64 and during runtime only the Windows Application itself is emulated through either Box86 oder FEX. Default for 32 bit exe is Box86, anyway choosing FEX crashes with Atomic Bomberman in the Wine 10.14 Build. 
...
Jsmon is a command line tool intended to pre-check all joystick names and buttons. Device list should update in realtime. It will kill itself by keeping a button + axis pressed for 2 seconds and output a specific return value (depending on direction / button combo). We can catch that value by a script and have the base for an autostart / zero-command-line-image - hopefully later. 
...
```
sudo chmod 777 /dev/uinput # or do it the proper way, but I don't care
...
It's said that Saturn Bomberman comes quite close to Hi-Ten Bomberman. They have implemented an anamorphic widescreen mode, in a way that when you connect the console to a 16:9 CRT and stretch the 4:3 image, everything will be displayed with correct aspect ratio. Should make no difficulties to play it with 10 keyboards...but needs a halfway recent PC.
...
* Compare Wine-Hangover with emulating the complete x86 build of standard Wine
...
A complete reimplementation of Atomic Bomberman (Original Game Contents needed) can be found here:
